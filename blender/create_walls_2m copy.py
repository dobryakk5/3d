...
def create_fill_walls_between_openings(wall_segments, openings, wall_thickness, scale_factor):
    ...
    и создает стены сверху и снизу от каждого проема
    ...
    # Сначала создаем стены сверху и снизу от каждого проема
    for opening in openings:
        opening_bbox = opening["bbox"]
        opening_orientation = opening.get("orientation", "horizontal")
        
        if opening_orientation == "horizontal":
            # Для горизонтального проема (окно/дверь)
            opening_x = opening_bbox["x"]
            opening_y = opening_bbox["y"]
            opening_width = opening_bbox["width"]
            opening_height = opening_bbox["height"]
            
            # Находим все сегменты стен на этой же линии (y координата)
            related_segments = []
            for segment in wall_segments:
                seg_bbox = segment["bbox"]
                seg_orientation = segment.get("orientation", "horizontal")
                
                if seg_orientation == "horizontal" and abs(seg_bbox["y"] - opening_y) < 5:
                    related_segments.append(segment)
            
            # Сортируем сегменты по x координате
            related_segments.sort(key=lambda s: s["bbox"]["x"])
            
            # Находим сегменты слева и справа от проема
            left_segment = None
            right_segment = None
            
            for segment in related_segments:
                seg_bbox = segment["bbox"]
                seg_right = seg_bbox["x"] + seg_bbox["width"]
                
                if seg_right <= opening_x and (left_segment is None or
                    seg_bbox["x"] + seg_bbox["width"] > left_segment["bbox"]["x"] + left_segment["bbox"]["width"]):
                    left_segment = segment
                
                if seg_bbox["x"] >= opening_x + opening_width and (right_segment is None or
                    seg_bbox["x"] < right_segment["bbox"]["x"]):
                    right_segment = segment
            
            # Создаем стену сверху проема (только если есть место)
            top_wall_height = wall_thickness * scale_factor
            # Проверяем, не выходит ли стена за границы сцены
            if opening_y - top_wall_height >= 0:
                top_segment = {
                    "segment_id": f"top_wall_{opening['id']}",
                    "bbox": {
                        "x": opening_x,
                        "y": opening_y - top_wall_height,
                        "width": opening_width,
                        "height": top_wall_height
                    },
                    "orientation": "horizontal"
                }
                fill_segments.append(top_segment)
            
            # Создаем стену снизу проема (только если есть место)
            bottom_wall_height = wall_thickness * scale_factor
            # Проверяем, не выходит ли стена за границы сцены
            if opening_y + opening_height + bottom_wall_height <= 2000:  # Предполагаем максимальную высоту сцены
                bottom_segment = {
                    "segment_id": f"bottom_wall_{opening['id']}",
                    "bbox": {
                        "x": opening_x,
                        "y": opening_y + opening_height,
                        "width": opening_width,
                        "height": bottom_wall_height
                    },
                    "orientation": "horizontal"
                }
                fill_segments.append(bottom_segment)
            
        else:  # vertical
            # Для вертикального проема (окно/дверь)
            opening_x = opening_bbox["x"]
            opening_y = opening_bbox["y"]
            opening_width = opening_bbox["width"]
            opening_height = opening_bbox["height"]
            
            # Находим все сегменты стен на этой же линии (x координата)
            related_segments = []
            for segment in wall_segments:
                seg_bbox = segment["bbox"]
                seg_orientation = segment.get("orientation", "horizontal")
                
                if seg_orientation == "vertical" and abs(seg_bbox["x"] - opening_x) < 5:
                    related_segments.append(segment)
            
            # Сортируем сегменты по y координате
            related_segments.sort(key=lambda s: s["bbox"]["y"])
            
            # Создаем стену слева от проема (только если есть место)
            left_wall_width = wall_thickness * scale_factor
            # Проверяем, не выходит ли стена за границы сцены
            if opening_x - left_wall_width >= 0:
                left_segment = {
                    "segment_id": f"left_wall_{opening['id']}",
                    "bbox": {
                        "x": opening_x - left_wall_width,
                        "y": opening_y,
                        "width": left_wall_width,
                        "height": opening_height
                    },
                    "orientation": "vertical"
                }
                fill_segments.append(left_segment)
            
            # Создаем стену справа от проема (только если есть место)
            right_wall_width = wall_thickness * scale_factor
            # Проверяем, не выходит ли стена за границы сцены
            if opening_x + opening_width + right_wall_width <= 3000:  # Предполагаем максимальную ширину сцены
                right_segment = {
                    "segment_id": f"right_wall_{opening['id']}",
                    "bbox": {
                        "x": opening_x + opening_width,
                        "y": opening_y,
                ...
                fill_segments.append(right_segment)
    
    # Теперь обрабатываем промежутки между сегментами стен (старая логика)
...
def create_3d_walls_from_json(json_path, wall_height=2.0, export_obj=True, clear_scene=True, brick_texture_path=None, render_isometric=False):
    ...
    try:
        ...
        # Создаем заполняющие стены для пустых пространств между проемами
